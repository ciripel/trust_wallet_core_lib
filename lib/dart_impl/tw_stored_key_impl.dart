...
class TWStoredKeyImpl extends TWStoredKey {
  static Pointer<Void>? importPrivateKey(
    ...
    final _password = TWData.TWDataCreateWithBytes(
        password.toPointerUint8(), password.length);
    final _pk = TWData.TWDataCreateWithBytes(pk.toPointerUint8(), pk.length);
    final _storedKey =
        TWStoredKey.TWStoredKeyImportPrivateKey(_pk, _name, _password, coin);
  ...
  static Pointer<Void>? importHDWallet(
    ...
    final _storedKey = TWStoredKey.TWStoredKeyImportHDWallet(
        _mnemonic, _name, _password, coin);
  ...
  static Uint8List? decryptPrivateKey(
    ...
    final _password = TWData.TWDataCreateWithBytes(
        password.toPointerUint8(), password.length);
    final _pivateKey =
        TWStoredKey.TWStoredKeyDecryptPrivateKey(storedKey, _password);
    TWData.TWDataDelete(_password);
    if (_pivateKey.address == 0) {
      return null;
    }
    return TWData.TWDataBytes(_pivateKey)
        .asTypedList(TWData.TWDataSize(_pivateKey));
  }
  ...
  static String? decryptMnemonic(Pointer<Void> storedKey, Uint8List password) {
    ...
    final _mnemonic =
        TWStoredKey.TWStoredKeyDecryptMnemonic(storedKey, _password);
    TWData.TWDataDelete(_password);
  ...
  static Pointer<Void> privateKey(
    ...
    final _password = TWData.TWDataCreateWithBytes(
        password.toPointerUint8(), password.length);
    final _privateKey =
        TWStoredKey.TWStoredKeyPrivateKey(storedKey, coinType, _password);
  ...
  static void addAccount(Pointer<Void> storedKey, String address, int coin,
      String derivationPath, String extetndedPublicKey) {
    ...
    TWStoredKey.TWStoredKeyAddAccount(
        storedKey, _address, coin, _derivationPath, _extetndedPublicKey);
