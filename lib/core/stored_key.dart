...
class StoredKey {
  ...
  static StoredKey? importPrivateKey(
    ...
    final _p = Uint8List.fromList(password.codeUnits);
    final pointer =
        TWStoredKeyImpl.importPrivateKey(privateKeyData, name, _p, coin);
    if (pointer == null) {
      return null;
    }
    _nativehandle = pointer;
  ...
  String? exportJson() {
    ...
    final bytes =
        TWData.TWDataBytes(_data).asTypedList(TWData.TWDataSize(_data));
  ...
  Pointer<Void> accountForCoin(int coin, HDWallet hdWallet) {
    return TWStoredKeyImpl.accountForCoin(
        _nativehandle, coin, hdWallet._nativehandle);
  }
  ...
  void addAccount(String address, int coin, String derivationPath,
    ...
    TWStoredKeyImpl.addAccount(
        _nativehandle, address, coin, derivationPath, extetndedPublicKey);
  }
  ...
  Uint8List? decryptPrivateKey(Uint8List password) {
    Uint8List? _data =
        TWStoredKeyImpl.decryptPrivateKey(_nativehandle, password);
    if (_data == null) {
      return null;
    }
    return _data;
