...
import 'package:trust_wallet_core_lib_example/base_example.dart';

class PrivateKeyIsValidExample extends BaseExample {
  final HDWallet wallet;
  const PrivateKeyIsValidExample(this.wallet, {Key? key})
      : super('PrivateKeyIsValid', key: key);

  @override
  _PrivateKeyIsValidExampleState createState() =>
      _PrivateKeyIsValidExampleState();
}

class _PrivateKeyIsValidExampleState
    extends BaseExampleState<PrivateKeyIsValidExample> {
  @override
  void initState() {
    super.initState();
    ...
    logger.d(
        '0：${PrivateKey.isValid(AIONPk.data(), 0)}、none：${PrivateKey.isValid(AIONPk.data(), TWCurve.TWCurveNone)}、ed25519：${PrivateKey.isValid(AIONPk.data(), TWCurve.TWCurveED25519)}、secp256k1：${PrivateKey.isValid(AIONPk.data(), TWCurve.TWCurveSECP256k1)}、ed25519Blake2bNano：${PrivateKey.isValid(AIONPk.data(), TWCurve.TWCurveED25519Blake2bNano)}、curve25519：${PrivateKey.isValid(AIONPk.data(), TWCurve.TWCurveCurve25519)}、nist256p1:${PrivateKey.isValid(AIONPk.data(), TWCurve.TWCurveNIST256p1)}、ed25519Extended:${PrivateKey.isValid(AIONPk.data(), TWCurve.TWCurveED25519Extended)}');

    /// BTC 原 private key
    final privateKeyStr =
        "L5CRwCGQeMD7tPySaV62fAYLNHEbVRJUeLn8GCuLLkaPeq4hYk8x";
    ...
    final isOk =
        PrivateKey.isValid(Uint8List.fromList(data), TWCurve.TWCurveED25519);
    logger.d(
        "privateKey:L5CRwCGQeMD7tPySaV62fAYLNHEbVRJUeLn8GCuLLkaPeq4hYk8x : $isOk");

    /// BTC 修改后 private key
    final privateKeyStrTemp =
        "L5CRwCGQeMD7tPySaV62fAYLNHEbVRJUeLn8GCuLLkaPeq4hYk80";
    final dataStrTemp =
        BTCdecode(privateKeyStrTemp, TWCoinType.TWCoinTypeBitcoin);
    if (dataStrTemp == null) {
      print("dataStrTemp null !!!");
      return;
    }
    List<int> dataTemp = hex.decode(dataStrTemp);
    final isOkTemp = PrivateKey.isValid(
        Uint8List.fromList(dataTemp), TWCurve.TWCurveED25519);
    logger.d(
        "之后privateKey:L5CRwCGQeMD7tPySaV62fAYLNHEbVRJUeLn8GCuLLkaPeq4hYk80 : $isOkTemp");
  }
